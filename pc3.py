...
#use regular expressions to find the desired string pattern!!

#for char in 'abcdefghijklmnopqrstuvwxyz':
	#match = re.search(pattern, string, flags=0)
	#upperChar = '[A-Z]'
	#searchString = upperChar + char + upperChar
	#print searchString
#	reString = re.compile(r'[A-Z]{3,5}?+[a-z]+[A-Z]{3,5}?')
#	match = reString.match(test_string)
#	print 'the current char is: %s match: %s' % (char, match)

#reString = re.compile(r'([a-z]{1})([A-Z])([A-Z])([A-Z])([a-z]{1})([A-Z])([A-Z])([A-Z])([a-z]{1})')
reString = re.compile(r'([a-z]{1})([A-Z]{3})([a-z]{1})([A-Z]{3})([a-z]{1})')
#match = re.search(reString, test_string)
matches = re.finditer(reString, test_string)
if matches:
	#print 'is there a match?: %s' % (match.group())
	for m in matches:
		print m.group()
else:
